...
export default function TradesTab({ trades, tradesLoading, pair, cleanHolders, holderCount, snipCount, bundleCount, top10HoldPercent, insiderPercent, devPercent, copiedKey, onCopy }) {
  ...
  return (
    <>
      ...
      {!trades.length ? (
        <>
          <div style={{ padding: '10px 12px', fontSize: 10, color: 'var(--text-muted)', borderBottom: '1px solid var(--border)' }}>
            {tradesLoading ? 'Loading trades…' : 'No trades yet — will refresh automatically.'}
          </div>
        ...
        <>
          ...
          <div style={{
            display: 'grid',
            gridTemplateColumns: '55px 55px 1fr 80px 80px 80px 80px',
            padding: '5px 12px',
            ...
            <div style={{ textAlign: 'right' }}>COPY</div>
          </div>
          {trades.slice(0, 100).map((trade, index) => {
            ...
            return (
              <div key={index} className="trade-row">
                <div style={{ color: isBuy ? 'var(--green)' : 'var(--red)', fontWeight: 700 }}>
                  {isBuy ? 'BUY' : 'SELL'}
                </div>
                <div style={{ color: 'var(--text-muted)' }}>{fAgeMs(trade.timestamp)}</div>
                <div style={{ color: 'var(--text-link)', fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {fAddr(trade.wallet)}
                </div>
                <div style={{ textAlign: 'right' }}>{fU(trade.volumeUsd)}</div>
                <div style={{ textAlign: 'right', fontFamily: 'monospace' }}>{fP(trade.priceUsd)}</div>
                <div style={{ textAlign: 'right', color: 'var(--text-muted)' }}>{fN(trade.amount)}</div>
                <div style={{ textAlign: 'right' }}>
                  <button
                    className="copy-btn"
                    onClick={() => onCopy(trade.wallet || '', copyKey)}
