...
function TokenLogo({ sym, url }) {
  if (url) return (
    <img src={url} alt="" style={{ width:20, height:20, borderRadius:'50%', objectFit:'cover', flexShrink:0, marginRight:6 }}
      onError={e => e.currentTarget.style.display='none'} />
  );
  return (
    <div style={{ width:20, height:20, borderRadius:'50%', background:'var(--border)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, fontWeight:700, flexShrink:0, marginRight:6 }}>
      {(sym||'?')[0]}
    </div>
  );
}
        ...
        <tbody>
          {rows.length ? rows.map(p => (
            <tr key={p.pairAddr} onClick={() => onScan(p.addr)} style={{ cursor:'pointer' }} className="scanner-row">
              <TD><div style={{ display:'flex', alignItems:'center' }}><TokenLogo sym={p.sym} url={p.imageUrl} /><div><strong style={{ color:'var(--text-bright)' }}>{p.sym}</strong><br /><span style={{ color:'var(--text-muted)', fontSize:9 }}>{p.name.slice(0,16)}</span></div></div></TD>
        ...
        <tbody>
          {rows.map((p, i) => (
            <tr key={p.pairAddr} onClick={() => onScan(p.addr)} style={{ cursor:'pointer' }}>
              ...
              <TD><div style={{ display:'flex', alignItems:'center' }}><TokenLogo sym={p.sym} url={p.imageUrl} /><div><strong style={{ color:'var(--text-bright)' }}>{p.sym}</strong><br /><span style={{ color:'var(--text-muted)', fontSize:9 }}>{p.name.slice(0,14)}</span></div></div></TD>
              <TD right>{fP(p.price)}</TD>
      ...
      <TableWrap>
        ...
        <tbody>
          {rows.length ? rows.map(p => {
            ...
            return (
              <tr key={p.pairAddr} onClick={() => onScan(p.addr)} style={{ cursor:'pointer' }}>
                <TD style={{ borderLeft:`2px solid ${sigColor}` }}><div style={{ display:'flex', alignItems:'center' }}><TokenLogo sym={p.sym} url={p.imageUrl} /><div><strong style={{ color:'var(--text-bright)' }}>{p.sym}</strong><br /><span style={{ color:'var(--text-muted)', fontSize:9 }}>{(p.name||'').slice(0,14)}</span></div></div></TD>
                <TD right>{fP(p.price)}</TD>
                <TD right>{chSpan(p.ch5m)}</TD><TD right>{chSpan(p.ch1h)}</TD><TD right>{chSpan(p.ch24)}</TD>
                <TD right>{fU(p.vol24)}</TD><TD right>{fU(p.liq)}</TD>
...
export default function TerminalView({ mode, onScan }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const REAL_SOL = 'So11111111111111111111111111111111111111112';
      const parsed = raw
        .filter(p => {
          const sym = (p.baseToken?.symbol || '').toUpperCase().trim();
          const addr = p.baseToken?.address || '';
          if (/^(SOL|WSOL|SOLANA)$/.test(sym) && addr !== REAL_SOL) return false;
          return true;
        })
        .map(p => enrichPair(parsePair(p)));
      setPairs(parsed);
