...
export async function fetchFeed() {
  ...
  // Filter fake SOL/WSOL/SOLANA impersonator tokens
  ...
  all.forEach(p => {
    ...
    if (/^(SOL|WSOL|SOLANA)$/.test(sym) && m !== REAL_SOL) return;
...
export async function fetchRugCheck(mint) {
  try {
    ...
    if (r.ok) {
      ...
      // Debug: log the structure so we can see insider fields
      // console.log('[RugCheck]', JSON.stringify(Object.keys(data)));
      return data;
    }
  } catch (_) {}
  return null;
