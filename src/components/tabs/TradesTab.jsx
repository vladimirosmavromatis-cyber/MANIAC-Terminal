              ...
              <div key={index} style={{
                ...
                <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  <a
                    ...
                    rel="noreferrer"
                    title={trade.wallet}
                    style={{ color: 'var(--text-link)', fontFamily: 'monospace', fontSize: 11, textDecoration: 'none', padding: '2px 6px', borderRadius: 4, border: '1px solid rgba(88,166,255,.25)', background: 'rgba(88,166,255,.08)' }}
                  >
                    {fAddr(trade.wallet)} ↗
                  </a>
                </div>
                <div style={{ textAlign: 'right', fontWeight: 700, color: isBuy ? 'var(--green)' : 'var(--red)' }}>{fU(trade.volumeUsd)}</div>
                <div style={{ textAlign: 'right', fontFamily: 'monospace', fontSize: 10 }}>{fP(trade.priceUsd)}</div>
                <div style={{ textAlign: 'right', color: 'var(--text-muted)', fontSize: 10 }}>{fN(trade.amount)}</div>
