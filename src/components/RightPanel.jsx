...
import { fU, fNum, walletName } from '../utils.js';
  ...
  return (
    <div style={{ overflowY: 'auto', borderLeft: '1px solid var(--border)', background: 'var(--bg-panel)', scrollbarWidth: 'thin' }}>
      <div style={{ padding: '8px 10px', fontSize: 10, fontWeight: 700, color: 'var(--text)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        Token Data & Security
        <span style={{ fontSize: 9, padding: '2px 6px', borderRadius: 3, background: riskColor + '22', color: riskColor, border: '1px solid ' + riskColor + '44' }}>{riskLabel}</span>
      </div>
      ...
      <div style={{ padding: '8px 10px', borderTop: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <span style={{ fontFamily: 'monospace', fontSize: 9, color: 'var(--yellow)' }}>{mint ? mint.slice(0, 8) + '…' + mint.slice(-4) : ''}</span>
          <button className="copy-btn" onClick={() => onCopy(mint, 'ca')} style={{ color: copiedKey === 'ca' ? 'var(--green)' : 'var(--text-muted)' }}>
            {copiedKey === 'ca' ? '✓' : '⎘'}
          </button>
          <a href={'https://solscan.io/token/' + mint} target="_blank" rel="noreferrer" style={{ color: 'var(--text-link)', fontSize: 10 }}>↗</a>
        </div>
      </div>
      ...
      {price > 0 && (
        <div style={{ padding: '8px 10px', borderTop: '1px solid var(--border)' }}>
          ...
          {[
            { label: 'STOP LOSS', sub: '-18%',     p: stopLoss, pct: '-18%',  color: 'var(--red)',    icon: '🔴' },
            { label: 'BUY ENTRY', sub: '-14%',     p: buyZone,  pct: '-14%',  color: 'var(--yellow)', icon: '🟡' },
            { label: 'TARGET 1',  sub: 'Sell 60%', p: target1,  pct: '+100%', color: 'var(--green)',  icon: '🎯' },
            { label: 'TARGET 2',  sub: 'Moonshot', p: target2,  pct: '+300%', color: 'var(--cyan)',   icon: '🚀' },
          ].map(({ label, sub, p, pct, color, icon }) => (
            <div key={label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '5px 8px', marginBottom: 4, borderRadius: 5, background: color + '18', border: '1px solid ' + color + '33' }}>
              <div>
                <div style={{ fontSize: 9, fontWeight: 800, color, display: 'flex', alignItems: 'center', gap: 4 }}>{icon} {label}</div>
      ...
      {mint && (
        <div style={{ padding: '8px 10px', borderTop: '1px solid var(--border)' }}>
          <a href={'https://x.com/search?q=' + encodeURIComponent(mint) + '&f=live'} target="_blank" rel="noreferrer"
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '7px 10px', borderRadius: 6, background: 'rgba(88,166,255,.07)', border: '1px solid rgba(88,166,255,.2)', color: 'var(--text-link)', textDecoration: 'none', fontSize: 10, fontWeight: 700 }}>
